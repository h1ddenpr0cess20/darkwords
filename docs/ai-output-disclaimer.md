...
This document explains the limits of responsibility for the Darkwords project ("the App"). It is informational and not legal advice. It supplements, and does not replace, the MIT License and any Terms of Service included in this repository or user interface.
...
## 2) Definitions
- "App": The Darkwords client-side interface and accompanying materials.
- "LLMs/Providers": Third-party large language models and services (e.g., Anthropic, OpenAI, or any model you reach through an LM Studio server) that you connect with your own credentials.
...
- You supply and control your own API keys and provider choices. Keys live in your browser and are sent directly to the provider endpoints (`api.anthropic.com`, `api.openai.com`, or your own LM Studio server).
...
- Not child-directed: The App is not designed for children and may produce content inappropriate for minors. Darkwords ships with a deliberately cold, contemptuous default personality and places no restrictions on the personas you can define.
...
- Tool calling and automation: If enabled, tools (web search, code interpreter, image generation, memory, skills, MCP servers, document search) may act on model instructions. Validate inputs/outputs and accept responsibility for results.
