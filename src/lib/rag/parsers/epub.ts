 ...
 */
export async function extractEpubText(arrayBuffer: ArrayBuffer): Promise<{ text: string; chapterCount: number }> {
  ...
  if (opfEntry) {
    ...
    if (opfFile) {
      ...
      const basePath = opfEntry.includes('/') ? opfEntry.slice(0, opfEntry.lastIndexOf('/') + 1) : '';
  ...
  for (const path of htmlFiles) {
    ...
    dom.querySelectorAll('script,style').forEach((el) => {
      el.remove();
    });
